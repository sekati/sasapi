...
class com.sekati.service.AmbientOrb {
	...
	public function config (colorId:String, animId:String, comment:String) {
		if (!_devId) {
			throw new Error ("@@@ com.sekati.service.AmbientOrb Error: Device ID not set");
			return;
		}
		if (!colorId) {
			throw new Error ("@@@ com.sekati.service.AmbientOrb Error: invalid config colorId arg");
			return;
		}
		if (!animId) {
			throw new Error ("@@@ com.sekati.service.AmbientOrb Error: invalid config animId arg");
			return;
		}
		if (!comment) {
			comment = "com.sekati.service.AmbientOrb";
		}
		var xm:XML = new XML ();
		xm.ignoreWhite = true;
